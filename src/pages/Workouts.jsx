    ...
    <div className="min-h-screen bg-brand-black px-4 py-16 text-brand-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {workouts.map((item) => (
            <div key={item.title} className="overflow-hidden rounded-[28px] border border-brand-white/10 bg-brand-white/8 backdrop-blur-xl">
              <img src={item.image} alt={item.title} className="h-44 w-full object-cover" />
              <div className="p-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold">{item.title}</h3>
                  <span className="rounded-full border border-brand-red/30 bg-brand-red/10 px-3 py-1 text-sm text-brand-red">{item.level}</span>
                </div>
                <p className="mt-2 text-sm text-brand-gray">{item.muscle}</p>
                <div className="mt-4 space-y-2 text-sm text-brand-gray">
                  <div className="flex items-center justify-between rounded-2xl bg-brand-white/10 px-3 py-2"><span>Sets</span><span>{item.sets}</span></div>
                  <div className="flex items-center justify-between rounded-2xl bg-brand-white/10 px-3 py-2"><span>Reps</span><span>{item.reps}</span></div>
                  <div className="flex items-center justify-between rounded-2xl bg-brand-white/10 px-3 py-2"><span>Rest</span><span>{item.rest}</span></div>
                </div>
              </div>
            </div>
          ))}
