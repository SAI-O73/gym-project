    ...
    <div className="min-h-screen bg-brand-black px-4 py-16 text-brand-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[32px] border border-brand-white/10 bg-brand-white/8 p-8 backdrop-blur-xl">
            <h3 className="text-2xl font-semibold">Mission</h3>
            <p className="mt-4 text-brand-gray">To make elite fitness coaching accessible through clean design, adaptive plans, and intelligent recommendations.</p>
            <h3 className="mt-8 text-2xl font-semibold">Vision</h3>
            <p className="mt-4 text-brand-gray">To build the most intuitive performance experience for people seeking healthier, stronger, and more consistent lifestyles.</p>
          </div>
          <div className="grid gap-4">
            {trainers.map((trainer) => (
              <div key={trainer.name} className="flex items-center gap-4 rounded-[24px] border border-brand-white/10 bg-brand-white/8 p-4 backdrop-blur-xl">
                <img src={trainer.image} alt={trainer.name} className="h-16 w-16 rounded-full object-cover" />
                <div>
                  <p className="font-semibold text-brand-white">{trainer.name}</p>
                  <p className="text-sm text-brand-gray">{trainer.role}</p>
                </div>
              </div>
            ))}
          </div>
